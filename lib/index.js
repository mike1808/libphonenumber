...
var isTypeMobile = function (validNumber) {
  return phoneUtil.getNumberType(validNumber) === PNT.MOBILE;
};
...
var validate = function (number, countryCode, callback) {
  ...
  try {
    result = validateNumber(number, countryCode);
  }
  catch (e) {
    error = e
  }
...
var getRegion = function (validNumber) {
    return phoneUtil.getRegionCodeForNumber(validNumber);
};

var intlParsed = function (validNumber) {
  return phoneUtil.format(validNumber, PhoneNumberFormat.E164)
};
